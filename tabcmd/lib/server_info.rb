...
class ServerInfo
  attr_accessor :no_prompt, :save_cookie, :timeout, :authenticity_token, :monitor, :silent
  ...
  def request_bootstrap(workbookView)
  ...
  def execute(request, opts = {})
    ...
    when Net::HTTPRedirection
      ...
      if location.path == '/auth/'
        ...
        # we should never ever Post to /views or /workbooks (redirects from succesful login to paid/public, so force a Get here
  ...
  def get_token_path
    ...
    begin
      unless Tabcmd::Test
        if ENV['APPDATA']
          dir = File.join(ENV['APPDATA'], 'Tableau')
          FileUtils.makedirs dir
          path = File.join(dir, 'tabcmd.txt')
        else
          logger.debug "Could not create token path because APPDATA environment variable is not set."
        end
      else
        if ENV['TEMP']
