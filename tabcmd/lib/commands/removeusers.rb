...
class Removeusers < MultiCommand::Command
  include Http_Util
...
File containing a list of users, one
                                     per line, to remove from the group.
EOM
          ) do |filename|
...
EOM
  ...
  def removeusers(group, file)
    logger.info "Removing users listed in #{@filename} to group #{group}..."
    ...
    params += [ text_to_multipart('do_remove', 'true') ]
  ...
  def run(opts,args)
    ...
    contents.each do |file|
      Server.login
      run_each(:removeusers, args, file)
    end
  end
end
