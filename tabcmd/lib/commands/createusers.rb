...
class Createusers < MultiCommand::Command
  include Http_Util
  ...
  def doc
    <<EOM
Create users.  The users are read from the given comma separated values (csv)
...
5. Administrator (system/content/none) (DEPRECATED: yes/true/1 or no/false/0)
...
Tabcmd waits for the createusers task to complete.  You may choose not to wait for the task
...
EOM
    ...
    opts.on( "--[no-]administrator", "DEPRECATED: do not use" ) do |val| # backwards-compatibility only
      @full_administrator = val
    end
    opts.on( "--admin-type TYPE", ["system", "content", "none"],
             <<EOM
Assigns [or removes] the Admin right
                                     for all users in the CSV file. This
                                     setting may be overridden by the values
                                     on individual rows in the CSV file.
                                     TYPE may be: system, content, or none
...
EOM
  ...
  def import(csv)
    real_admin = "none"
    real_admin = (@full_administrator ? "system" : "none") unless @full_administrator.nil?
    real_admin = @admin_type unless @admin_type.nil?
    contents = FileUtil::get_file_ensure_utf8(csv)
    request = Server.create_request("manual/upload_action/system_users", "Post")
    ...
    logger.info "Adding users listed in #{csv} to server..."
