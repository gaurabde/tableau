  ...
  private
    ...
    def raise_if_environment_is_in_excluded_environments
      if environment_is_in_excluded_environments?
        raise FixtureReplacement::InclusionError, "FixtureReplacement cannot be included in the #{Object.const_get(:Rails.env)} environment!"
      end
    end

    def environment_is_in_excluded_environments?
      return false unless defined?(Rails.env)
      excluded_environments.include?(Rails.env) ? true : false
    end

    def rails_root
      defined?(Rails.root) ? Rails.root : nil
    end
  end
end
